...
def find_and_analyze_experiments():
    ...
    for exp_folder in experiment_folders:
        ...
        if best_is_save_path.exists():
            logger.info(
                f"Skipping {exp_folder} - best_real_inception_score.csv already exists"
            )
            ...
            try:
                ...
                logger.warning(
                    f"Could not read existing best file {best_is_save_path}: {e}"
                )
        ...
        if inception_scores is not None and len(inception_scores) > 0:
            ...
            logger.info(
                f"Saved statistics for {experiment_name} to {best_is_save_path}"
            )
    ...
    if results:
        ...
        best_by_optimizer = summary_df.groupby("optimizer")["best_is"].agg(
            ["max", "mean", "count"]
        )
        print(best_by_optimizer)
        print("\nAverage IS by optimizer:")
        avg_by_optimizer = summary_df.groupby("optimizer")["avg_is"].agg(
            ["max", "mean", "count"]
        )
