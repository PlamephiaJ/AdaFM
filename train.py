...
def run(cfg: DictConfig) -> None:
    ...
    args = argparse.Namespace(dataroot=cfg.datasets.dataroot, dataset=cfg.datasets.name, download=cfg.datasets.download, batch_size=cfg.models.training.batch_size)
    ...
    if cfg.models.name == "wgan":
        ...
        generator = Generator(channels=cfg.models.generator.channels, in_dim=cfg.models.generator.z_dim).to(device)
        discriminator = Discriminator(channels=cfg.models.discriminator.channels).to(device)

        if cfg.optimizers.name == 'adam':
            d_optimizer = torch.optim.Adam(discriminator.parameters(), lr=cfg.optimizers.lr, betas=(cfg.optimizers.b1, cfg.optimizers.b2))
            g_optimizer = torch.optim.Adam(generator.parameters(), lr=cfg.optimizers.lr, betas=(cfg.optimizers.b1, cfg.optimizers.b2))
            ...
            d_optimizer = TiAda(discriminator.parameters(), beta=cfg.optimizers.beta, lr=cfg.optimizers.lr_y)
            g_optimizer = TiAda(generator.parameters(), beta=cfg.optimizers.beta, opponent_optim=d_optimizer, lr=cfg.optimizers.lr_x)
        elif cfg.optimizers.name == 'RSGDA':
            from optimizers.RSGDA import RSGDA
            d_optimizer = RSGDA(discriminator.parameters(), beta_y=cfg.optimizers.beta_y, lr_y=cfg.optimizers.lr_y)
            g_optimizer = RSGDA(generator.parameters(), beta_x=cfg.optimizers.beta_x, opponent_optim=d_optimizer, lr_x=cfg.optimizers.lr_x)
        elif cfg.optimizers.name == 'VRAdaGDA':
            from optimizers.VRAdaGDA import VRAdaGDA
            d_optimizer = VRAdaGDA(discriminator.parameters(), beta_y=cfg.optimizers.beta_y, lr_y=cfg.optimizers.lr_y)
            g_optimizer = VRAdaGDA(generator.parameters(), beta_x=cfg.optimizers.beta_x, opponent_optim=d_optimizer, lr_x=cfg.optimizers.lr_x)
        else:
            raise NotImplementedError(f"Optimizer {cfg.optimizers.name} is not implemented.")
