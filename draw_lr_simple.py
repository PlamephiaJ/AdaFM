...
def plot_individual_experiment(exp_name, log_file_x, log_file_y):
    ...
    if log_file_x:
        try:
            ...
            ax.plot(df_filtered['step'], df_filtered['learning_rate'], 
                   label="Generator", linewidth=2, alpha=0.8, linestyle='-', color='blue')
    ...
    if log_file_y:
        try:
            ...
            ax.plot(df_filtered['step'], df_filtered['learning_rate'],
                   label="Discriminator", linewidth=2, alpha=0.8, linestyle='--', color='red')
        except Exception as e:
            print(f"Error reading discriminator log for {exp_name}: {e}")
    
    # Customize plot
    ax.set_title(f'Learning Rate Evolution - {exp_name.replace("_", " ").title()} (First {MAX_STEPS//1000}K Steps)', fontsize=16)
...
def plot_overview():
    ...
    ax.set_title(f'Learning Rate Evolution - All Experiments (First {MAX_STEPS//1000}K Steps)', fontsize=16)
    ax.set_xlabel('Training Step', fontsize=14)
    ax.set_ylabel('Learning Rate', fontsize=14)
...
def quick_plot():
    ...
    if INDIVIDUAL_PLOTS:
        for exp_name, files in experiments.items():
            if files['x'] is not None or files['y'] is not None:  # At least one file exists
